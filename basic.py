...
def compile_statement(stmt, strings): 
    ...
    if cmd == "GRID":
        w_expr = tokens[1].rstrip(",")
        h_expr = tokens[2]
        code = compile_expr(w_expr)
        code.append("  MOV R3, R0")
        code.extend(compile_expr(h_expr))
        code.append("  MOV R1, R0")
        code.append("  MOV R0, R3")
        code.append("  SYS 10")
        return code
    ...
    if cmd == "GSET":
        ...
        code.extend(compile_expr(y_expr))
        code.append("  MOV R1, R0")
        code.extend(compile_expr(v_expr))
        code.append("  MOV R2, R0")
        code.append("  MOV R0, R3")
        code.append("  SYS 13")
        return code
    ...
    if cmd == "AGET":
        var = tokens[1].rstrip(",")
        i_expr = tokens[2]
        code = compile_expr(i_expr)
        code.append("  SYS 20")
        code.append(f"  STM R0, 0x{var_addr(var):04X}")
        return code
    
    if cmd == "ASET":
        i_expr = tokens[1].rstrip(",")
        v_expr = tokens[2]
        code = compile_expr(i_expr)
        code.append("  MOV R3, R0")
        code.extend(compile_expr(v_expr))
